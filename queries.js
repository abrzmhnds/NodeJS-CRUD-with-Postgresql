...
const createStudent = (request, response) => {
    ...
    pool.query('INSERT INTO CONTACT (nickname, address) VALUES ($1, $2)', [nickname, address], (error, results) => {
        if (error) {
            throw error
        }
...
const updateStudent = (request, response) => {
    ...
    try {
        const { nickname, address } = request.body;
        pool.query('UPDATE CONTACT SET nickname = $1, address = $2 WHERE id = $3', [nickname, address, id], (error, results) => {
            if (error) {
                throw error
            }
            // responseReturn.status = true;
